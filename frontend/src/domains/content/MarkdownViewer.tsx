...
const generateAnchorId = (text: string, index: number): string => {
  ...
  const extractTocFromMarkdown = (md: string): TocItem[] => {
    ...
    lines.forEach((line) => {
      ...
      if (trimmed.startsWith('# ')) {
        ...
        toc.push({ id: generateAnchorId(text, headingIndex++), text, level: 2 })
      } else if (trimmed.startsWith('### ')) {
        headingIndex++
      }
    })
      ...
      <ReactMarkdown
        ...
        components={{
          h1: ({ children, ...props }) => {
            ...
            const id = generateAnchorId(text, headingIndex++)
            return (
              <h1
                id={id}
          ...
          pre: ({ children, ...props }) => (
            <pre className="bg-slate-100 dark:bg-slate-950 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-800 p-4 rounded-lg font-mono text-xs overflow-x-auto leading-normal whitespace-pre my-4 select-none" {...props}>
              {children}
            </pre>
          ),
          ...
          img: ({ src, alt, ...props }) => (
            <span className="my-4 flex justify-center">
              <img src={src} alt={alt} className="max-w-full h-auto border border-gray-200 dark:border-slate-800 shadow-sm" {...props} />
            </span>
          ),
