...
const ProfileEditModal: React.FC<ProfileEditModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPassword('');
      setConfirmPassword('');
      // Refresh profile data reference
      setProfileData(data);
      ...
      const msg = err.response?.data?.message || err.response?.data || '프로필 수정 중 오류가 발생했습니다.';
      setErrorMsg(typeof msg === 'string' ? msg : '프로필 수정 중 오류가 발생했습니다.');
    } finally {
      setModalLoading(false);
    }
  };
