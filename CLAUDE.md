# A-Man 프로젝트 CLAUDE 개발 지침 (CLAUDE.md)

이 문서는 AI 에이전트(CLAUDE CLI)가 **A-Man (AssetERP 매뉴얼 시스템)** 프로젝트를 개발, 유지보수 및 협업할 때 준수해야 하는 핵심 가이드라인과 설계 원칙을 정리한 지침서입니다.
...
3. **임시 파일 경로**: 스크래치 스크립트나 임시 파일은 반드시 `.CLAUDE/` 내부가 아닌 프로젝트의 임시 디렉토리 혹은 지정된 에이전트 스크래치 경로를 사용합니다.
